<?php
/**
 * Cart Page (Child Theme Override)
 *
 * Orijinal WooCommerce şablonundan kopyalandı ve otomatik quantity update
 * JS'i (cart-auto-update.js) ile uyumlu çalışması için herhangi bir
 * yapısal değişiklik yapılmadı. İleri seviye özelleştirmelerde diff'i
 * minimum tutmaya çalışın ki gelecekte WooCommerce güncellemeleri ile
 * çakışma riski azalsın.
 *
 * @see     https://woocommerce.com/document/template-structure/
 * @package WooCommerce\Templates
 * @version 10.1.0 (eşitlendi)
 */
...
<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>" data-cart-item-key="<?php echo esc_attr( $cart_item_key ); ?>">
						...
						if ( $_product->is_sold_individually() ) {
							...
							$min_quantity = 1; // Min quantity child override
	...
	<div class="cart-grid-side">
		<?php do_action( 'woocommerce_before_cart_collaterals' ); ?>
		<div class="cart-collaterals">
	  <?php do_action( 'woocommerce_cart_collaterals' ); ?>
		</div>
		<div id="cart-status-anchor"></div>
